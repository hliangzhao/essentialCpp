...
1、使用标准库`cstdlib`提供的`exit(-1)`函数中止程序。更好的方式使用**异常处理**，后面会讲到。
...
```
这里用`const`修饰函数的返回值是值得思考的（见注释部分）。下面对`const`的使用做个梳理。

被`const`修饰的对象都受到强制保护，可以预防意外的变动，能提高程序的健壮性。
...
而复制也非常快，“值传递”和“引用传递”的效率几乎相当。除了值传递和引用传递，驶入参数若是指针，则`const`可起到防止意外改动该指针的作用。例如`void StrCpy(char *strDestination, const char *strSource)`。
