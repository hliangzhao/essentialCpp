...
我们需要准备两个类，一个是`BinTreeNode`，用于存储当前节点的数值`_val`、当前数值被插入次数`_count`（我们要求一个值只能在二叉树中出现一次，
...
我们可以将比较运算符从中提取出来：
```C++
// 此时可讲任意一种二元比较操作应用于两个同为elemType类型的对象上
...
```
那么就可以通过`LessThan<string, StrLt> func_lt`来定义函数对象了。
